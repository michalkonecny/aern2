...
proverOptions = ProverOptions
  ...
  <*> switch
    (
      long "proving-process"
      <> short 'a'
      <> help "Add this option if the target file was generated by our proving process (this option supersedes why3-mode)"
    )
  <*> switch
    (
      long "ce-mode"
...
runProver proverOptions@(ProverOptions why3Mode provingProcess ceMode depthCutoff bestFirstSearchCutoff p filePath) =
  do 
    if provingProcess
      then do
        ...
        case parseDRealSmtToF parsedFile of
          (Just vc, typedVarMap) ->
            let ecnf = minMaxAbsEliminatorECNF . fToECNF . simplifyF $ FNot vc -- Original vc has been negated by the proving process. Do this again to `undo` the negation
            in do
...
decideECNFWithVarMap ecnf typedVarMap (ProverOptions why3Mode provingProcess ceMode depthCutoff bestFirstSearchCutoff p filePath) = do
  result <- 
    if ceMode
      then
