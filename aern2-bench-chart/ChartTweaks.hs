...
scaledAxisExtraXSteps :: RealFloat a => LinearAxisParams a -> (a,a) -> [a] -> AxisFn a
scaledAxisExtraXSteps lap rs@(minV,maxV) extraXSteps ps0 =
  makeAxis' realToFrac realToFrac
  ...
  where
    ...
    labelvs   =
      extraXSteps ++
      (map fromRational $ steps (fromIntegral (_la_nLabels lap)) r)
    tickvs    =
      extraXSteps ++
      (map fromRational $ steps (fromIntegral (_la_nTicks lap))
