...
genericMaximum f dfs bts lBall rBall =
  ...
  sgn x =
    case x > 0 of
      ...
      Nothing -> Nothing
  tryFindSign :: Rational -> Integer -> (Integer, Maybe Integer)
  tryFindSign x n =
    let
      val = iterateUntilFixed (fst $ fromJust $ Map.lookup n dfs) (mpBallP (getPrecision lBall) x) -- TODO: precision
      sg  = sgn val
    in
          ...
          SearchInterval a b v ov k ts ->
            ...
            if isNothing $ signVars ts then -- TODO avoid recomputation of sign variations
              ...
              let
                ...
                (dl, sgnL) = tryFindSign a dm -- sgn $ evalDirect f' (mpBall l)
                (dm', sgnM) = tryFindSign m dl -- sgn $ evalDirect f' (mpBall m)
