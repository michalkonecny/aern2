...
remIntPoly p q = fromFracPoly $ MP.remPoly (toFracPoly p) (toFracPoly q)               
                    
derivative :: IntPoly -> IntPoly
derivative (IntPoly ts) = IntPoly $ Map.filterWithKey (\k _ -> k >= 0) $ Map.mapKeys (\k -> k - 1) $ Map.mapWithKey (\p c -> c*p) ts                    
