...
cheb2Power chebP@(Cheb.Poly ts) = 
    Power.Poly $ Map.filterWithKey (\k _ -> k <= Cheb.terms_degree ts) nts
    where
    (Power.Poly nts) = 
        (aux 0 0) * (PolyVec (Power.fromIntegerListP p [(0,1)]) (Power.fromIntegerListP p [(1,1)]))
    p = getPrecision chebP
    d = integer (integerLog2 $ Cheb.terms_degree ts + 1)  + 1
    dm1 = d - 1
    aux j i = 
        if j == dm1 then
          PolyCoVec 
            (Power.fromList [(0,Cheb.terms_lookupCoeff ts $ 2*i)]) 
            (Power.fromList [(0,Cheb.terms_lookupCoeff ts $ 2*i + 1)])
        else
          aux (j + 1) (2*i) + (aux (j + 1) (2*i + 1)) * (recPowers p) !!! (dm1 - j)
                            
recPowers :: Precision -> [PolyMat]
recPowers p = 
    mat : accu mat
    where
...
recPowersInt = 
    ...
    mat = 
        IntPolyMat 
            (IntPower.fromList [(0,0)]) 
            (IntPower.fromList [(0,1)]) 
            (IntPower.fromList [(0,-1)]) 
            (IntPower.fromList [(1,2)])
