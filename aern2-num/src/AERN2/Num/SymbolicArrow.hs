{-# LANGUAGE Arrows, GeneralizedNewtypeDeriving, OverloadedStrings, TypeOperators, FlexibleContexts, TypeSynonymInstances, FlexibleInstances, RankNTypes #-}
module AERN2.Num.SymbolicArrow where

import AERN2.Num.Operations
import AERN2.Num.Accuracy
import AERN2.Num.MPBall
import AERN2.Num.CauchyReal hiding (pi)
import qualified AERN2.Num.CauchyReal as CR

import Control.Arrow
import qualified Data.Map as Map

_expr1 ::  RealExpr
_expr1 = cauchyReal2exprNamed "pi" CR.pi
...
_expr3Direct x ac =
    cauchyReal2ball (realExpr2arrow _expr3 (Map.fromList [("x",cauchyReal x)])) ac

_pred1Direct :: Rational -> Bool
_pred1Direct x =
    (realPred2arrow _pred1 (Map.fromList [("x",cauchyReal x)]))

{- TODO define TH quasi-quotations, so that we can do eg:
...
proc (x,y) -> (realExpr2arrow ((var "x") + (var "y")) -< (Map.fromList [("x",x), ("y",y)]))
proc (x,y) -> (realPred2arrow ((var "x") > (var "y")) -< (Map.fromList [("x",x), ("y",y)]))
